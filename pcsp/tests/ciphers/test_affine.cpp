...
// Standalone test harness for AffineCipher (REQ-CIP-402).
...
int main() {
    ...
    std::vector<uint8_t> key       = { /* TODO: sample key, format: 2 bytes -- pair (a, b) */ };

    std::vector<uint8_t> ciphertext = cipher.encrypt(plaintext, key);
    std::vector<uint8_t> recovered  = cipher.decrypt(ciphertext, key);

    std::cout << "TODO: assert recovered == plaintext for AffineCipher" << std::endl;
